...
- ✅ **Cross-repo fan-out** — discover the npm projects under a parent folder (no
  config; monorepo child-folder manifests found by walking, others pinnable), then
  fan one operation across all of them as **N per-repo Changes** — dry-run first,
  apply on confirm, each repo keeping its own undo journal; repos where the op
  doesn't apply are skipped, not errored. State (scanned parents + manual pins)
  lives in the user's config dir, never in a scanned repo. Drivable **headless**
  (`visual-config fleet`), in the **browser** (a Fleet view with a folder
  browser), or by an **agent** (`fleet_discover`/`fleet_plan`/`fleet_apply`/
  `fleet_pin` MCP tools).
- 🔜 Not yet: cross-workspace version alignment, guided framework-config
  _editing_ (AST rewrite), IDE extensions, and the **hosted "point at any repo →
  PR"** flow (the diff playground is live).
