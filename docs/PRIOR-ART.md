...
> and precisely where Facet is different. Researched July 2026.
...
swaps, and (d) an MCP surface for agents. The delivery pattern Facet wants —
...
- **Gap Facet fills:** it is **read-only**, **ESLint-only**, and has **no MCP**.
  Facet generalizes this exact, proven UX to *editable* forms across *many*
...
- **Delivery:** `npx npm-gui@latest` → opens in browser. **The single closest
  delivery-model match to Facet.**
- **Alive?** **No** — last commit Sept 2023 (~669★). Effectively abandoned.
- **Gap Facet fills:** purely dependency-focused — no vuln→migrate, no config
  forms, no swaps, no MCP. Its abandonment is a market opening Facet can
...
- **Delivery:** a local server + an iframe UI shown **both** in-browser and
  **embedded in VS Code** via a webview. This is the exact "one engine, many
  faces (browser + IDE)" architecture Facet needs.
- **Alive?** Yes — very active, first-party Nuxt.
- **Gap Facet fills:** Nuxt-specific and runtime-app-focused, not config. But
  its `devtools-kit` / server+iframe approach is the blueprint for Facet's
...
| Tool | What | Delivery | Alive? | Gap Facet fills |
|---|---|---|---|---|
| **taze** | Modern update checker w/ interactive selection, monorepo-aware, **maturity-period** filtering (skip too-new releases) | CLI + TUI (`npx taze`) | ✅ active (~4.2k★, antfu) | Terminal-only, updates-only. Facet's visual upgrade flow competes directly — *adopt taze's maturity-window safety idea.* |
| **npm-check-updates (ncu)** | The de-facto "bump everything" tool; rewrites `package.json` ranges | CLI | ✅ very active | No UI, no vuln context, no migration guidance, no safety gating. Facet wraps this class of action behind a diff. |
| **npm-check** | Interactive outdated/unused checker | CLI/TUI | ⚠️ largely stale | Terminal-only, dated. |
| **knip** | Finds unused files/deps/exports (successor to depcheck) | CLI + VS Code ext + LSP + **`@knip/mcp`** | ✅ very active (~11.7k★) | Report-oriented, not a management surface. Notably *does* ship an MCP — a signal the agent angle is live. Facet can surface knip's findings. |
...
|---|---|---|---|---|
| **Version Lens** | Inline latest/available versions + one-click bump in manifests | VS Code ext | ✅ active on GitLab (~1M+ installs) | Editor gutter only, read-oriented, versions only. Proves demand for Facet's IDE phase. |
| **Import Cost** | Inline bundle size per import | VS Code ext | ⚠️ stale (last push 2024) | Signal-only, no actions. Facet shows size as one of many catalog signals. |
...
|---|---|---|---|---|
| **npm audit** | Built-in advisory scan against the npm advisory DB | CLI (JSON output) | ✅ first-party | A **data source** for Facet's vuln view, not a UI. |
...
| **Socket.dev** | Supply-chain risk (install scripts, obfuscation, network, typosquats) beyond CVEs | App + GitHub bot + CLI + API | ✅ active, well-funded | Overlaps Facet's *install-time safety* goal. Could be a signal source; also the most serious adjacent commercial player on "package trust." |
...
| **npq** | "Safer `npm install`" — pre-install audit (install scripts, age, downloads, advisories) then hands off | CLI wrapper | ✅ active (ships an `AGENTS.md`) | Install-time CLI gating, not visual. A candidate **data source** for Facet's catalog trust signals. |
| **Renovate** | Automated dependency-update PRs, deeply configurable | App/self-host bot | ✅ active (Mend) | Async, PR-based, CI-oriented. Complementary — Facet is the *interactive local* surface; Renovate is the *unattended background* one. |
...
|---|---|---|---|---|
| **publint** | Lints publishing config — `exports`/`main`/`types`/`files`/`bin`, deprecated fields | web + CLI + **programmatic API** + VS Code ext | ✅ active | **Embed directly** in Facet's publish-setup flow; show warnings with one-click fixes. Its multi-surface delivery is a model for Facet's own. |
...
transformers — exactly the primitives Facet wraps behind a previewed,
...
| **`biome migrate eslint` / `prettier`** | ESLint/Prettier settings → `biome.json` | into Biome only | The closest analog to a Facet "swap" button — but CLI-only, one-directional. |
...
General codemod **engines** (Facet would orchestrate, not rebuild these):
...
- **Why it matters:** it is the *only* project found that couples a UI **and**
  an MCP server around code transformation — the same triad Facet wants.
- **Gap Facet fills:** codemod.com is scoped to **code refactors / migration
  ...
  competitor and the one to position against — but it does not occupy Facet's
  surface. Watch it closely.

## MCP servers for package/config management (Facet's core moat)
...
| **npm-helper-mcp** | dep updates (wraps ncu) + search | Closest to Facet's *update* action — headless, ~8★. |
| **npm-run-mcp-server** | exposes `package.json` scripts as agent tools | Facet's script-runner idea, as an MCP, ~3★. |
...
Facet's clearest, most defensible moat, and the ecosystem's momentum (Socket,
...
|---|---|---|
| **create-vite / create-next-app / create-t3-app** | Interactive scaffolders at **project birth** | Write-once then exit. Facet is the **day-2+** tool they leave behind; its swap forms do continuously what they do once. |
| **@tsconfig/bases** | Shareable base tsconfigs (`@tsconfig/strictest`, etc.) | Offer as **one-click presets** in Facet's tsconfig editor. |
| **@antfu/eslint-config** | Opinionated all-in-one ESLint preset replacing eslint+prettier | Represents the "one preset" philosophy Facet's swap feature can apply/remove. |
| **Biome / ESLint flat-config migrators** | Official migration commands (`biome migrate eslint`, ESLint flat-config migrator) | **Wrap these** as the engine behind Facet's tooling-swap (behind a diff, honest about lossy rule translation). |
...
- **Vite `vite-plugin-inspect`**, **Vue DevTools**, **Nuxt DevTools** — all use
  ...
  for Facet's shared engine + multi-face delivery.

---

## What is genuinely novel about Facet
...
2. **Cross-tool, not single-tool.** Everyone else covers one tool (ESLint, or
   TS, or publishing). Facet is the *unifying surface*.
...
4. **An MCP server for config actions.** knip ships `@knip/mcp` (reporting) and
   ...
   *mutations* to agents.** This is Facet's strongest, most defensible moat and
...
5. **Framework- and dev-server-independent.** Every devtools *dashboard* that
   ...
   **coupled to a running dev server and locked to one framework/bundler**, and
   centers on *runtime/build debugging*, not the config lifecycle. Facet runs
...
- **Socket.dev**: best-funded player on package trust/supply-chain; likely to
  keep expanding the "safe install" surface Facet also wants to own.
- **First-party tools adding their own UIs** (an official Biome or oxc config
  UI, or npm/pnpm shipping a GUI) would erode single areas — Facet's defense is
  breadth + MCP + non-destructive editing, not any one tool's depth.
- **knip's MCP** shows the agent-tooling idea is already in motion; Facet needs
