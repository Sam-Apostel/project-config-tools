...
<folder>`). Because it runs on your machine it can open _many_ repos at once: point at a parent
  ...
  repo keeps its own undo journal. Repos where the change doesn't apply are skipped, not errored
...
🟡 **Not yet:** MCP fan-out tools (the fleet's browser + CLI + daemon ship today),
